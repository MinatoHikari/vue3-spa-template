...
const matchRule = (val: string, path: string, title?: string) => {
    if (path.toUpperCase().includes(val.toUpperCase())) return true;
    if (camelcase(path.replaceAll('/', '-')).includes(camelcase(val.replaceAll('/', '-'))))
        return true;
    if (!val.match(/[a-zA-Z\/]+/) && title?.includes(val)) return true;
    if (val.includes('/')) {
