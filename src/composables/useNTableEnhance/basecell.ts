...
const useInput = <Row extends Record<string, unknown>>(
...
const initCache = <Row extends Record<string, unknown>>(
    hook: () => {
        row: EnhanceRow<Row>;
        key: keyof Row;
    },
...
export const editableCell = <Row extends Record<string, unknown>>({
    row,
    key,
    component,
    wrapperProps,
