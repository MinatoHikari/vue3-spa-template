...
export interface ResponseHandlerConfig<T, R = T, RT = void, RR = void> {
    ...
    t?: ComposerTranslation<object, Locales.enUS>
}
...
function authErrorCallback(code: number) {
...
function getMessage(customMsg: boolean | string, msg?: string) {
 ...
 */
async function handleBlob<T>(blob: Blob) {
 ...
 */
export async function resHandler<T, R = string[] | undefined, RT = void, RR = void>(
    ...
    if (err) {
        ...
        if ([40100].includes(code)) authErrorCallback(code)
...
export async function blobHandler<T, R = string[] | undefined, RT = void, RR = void>(
...
export function httpErrorHandler(error: any) {
    ...
    if (error.isAxiosError) {
        ...
        if (err.response?.status === 404) globalMessage.error(i18nGlobal.t('request.error.404'))

        if (err.response?.status === 403) globalMessage.error(i18nGlobal.t('request.error.403'))
    }
}
