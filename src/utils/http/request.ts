...
function dataFormatter<T extends UnknownCommonKeyRecord>(data?: T, config: FilterNullConifg = {}) {
    if (!data)
        return data
...
function getFormContentType(formType?: 'mutipart' | 'urlencoded') {
...
function baseRequest<T = Record<string, any>>(config: RequestConfig): Promise<FilteredRequestResult<T>> {
...
export function request<T = Record<string, any>>(url: string, config: Omit<RequestConfig, 'url'>) {
...
export function getRequest<T = Record<string | number, unknown>, R = Record<string, any>>(url: string, data?: T | Nullable<T>, config: Omit<RequestConfig, 'url' | 'method' | 'params'> = {}) {
...
export function deleteRequest<T = Record<string | number, unknown>, R = Record<string, any>>(url: string, data?: T | Nullable<T>, config: Omit<RequestConfig, 'url' | 'params' | 'method'> = {}) {
    const { filterConifg } = config
    return baseRequest<R>({
        url,
        method: 'delete',
...
export function postRequest<T = Record<string | number, unknown>, R = Record<string, any>>(url: string, data?: T | Nullable<T>, config: Omit<RequestConfig, 'url' | 'data' | 'method'> = {}) {
    const { filterConifg } = config
    return baseRequest<R>({
        url,
        method: 'post',
...
export function postForm<T = Record<string | number, unknown>, R = Record<string, any>>(url: string, data?: T | Nullable<T>, config: Omit<RequestConfig & FormExtraParams, 'url' | 'data' | 'method'> = {}) {
...
export function putRequest<T = Record<string | number, unknown>, R = Record<string, any>>(url: string, data?: T | Nullable<T>, config: Omit<RequestConfig, 'url' | 'data' | 'method'> = {}) {
    const { filterConifg } = config
    return baseRequest<R>({
        url,
        method: 'put',
...
export function putForm<T = Record<string | number, unknown>, R = Record<string, any>>(url: string, data?: T | Nullable<T>, config: Omit<RequestConfig & FormExtraParams, 'url' | 'data' | 'method'> = {}) {
    const { formType, filterConifg } = config
    const formattedData = dataFormatter(data as UnknownCommonKeyRecord, filterConifg ?? {})
    let form: string | FormData
    if (!formType || formType === 'mutipart')
